...
export default function Component() {
  ...
  const handleToggle = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const localTheme = await store.get("theme") || "light"; 
    alert("Theme changed");

    alert(localTheme);

    alert("Theme changed");
    alert(localTheme.toString());
  ...
  return (
    <div className="flex min-h-screen flex-col md:flex-row">
      ...
      <div className="flex flex-1 flex-col justify-center bg-gradient-to-r from-purple-700 via-sky-500 to-blue-500 background-animate p-8 text-white">
        <h1 className="mb-4 text-4xl font-bold">Welcome To Xira!</h1>
  ...
  <svg
   ...
   {LoggedIn ? <p>Logging you in.</p>: <Auth initialAuthChoice="login"/> }
</div>
    </div>
  );
}
